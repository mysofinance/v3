...
import type { Router, Escrow, MockERC20, MockERC20Votes, MockOracle } from '../types/ethers-contracts/index.js'
...
describe('Router And Escrow Interaction', function () {
  ...
  describe('Bid on Auction', function () {
    it('should allow bidding on an auction', async function () {
      ...
      await settlementToken.connect(user1).approve(router.target, ethers.parseEther('100'))
      ...
      await expect(router.connect(user1).bidOnAuction(escrowAddress, user1.address, relBid, refSpot, data)).to.emit(
        router,
        'BidOnAuction',
      )
    })
    ...
    it('should revert if bidding with insufficient premium', async function () {
      ...
      await expect(
        router.connect(user1).bidOnAuction(escrowAddress, user1.address, lowRelBid, refSpot, data),
      ).to.be.revert(ethers)
    })
  })
  ...
  describe('Take Quote', function () {
    it('should allow taking a quote', async function () {
      ...
      // Approve tokens
      await settlementToken.connect(user1).approve(router.target, ethers.parseEther('1000000'))
      await underlyingToken.connect(owner).approve(router.target, ethers.parseEther('100'))

      // Take the quote
      await expect(router.connect(owner).takeQuote(owner.address, rfqInitialization, ethers.ZeroAddress)).to.emit(
        router,
        'TakeQuote',
      )
    })
    ...
    it('should revert if quote is expired (2/2)', async function () {
      ...
      const takeQuotePreview: any = await router.previewTakeQuote(rfqInitialization, ethers.ZeroAddress)
  ...
  describe('Withdraw', function () {
    it('should allow owner to withdraw after auction expiry', async function () {
      ...
      // Withdraw funds
      await expect(
        router.connect(owner).withdraw(escrowAddress, owner.address, underlyingToken.target, ethers.parseEther('100')),
    ...
    it('should revert if non-owner tries to withdraw', async function () {
      ...
      await expect(
        router.connect(user1).withdraw(escrowAddress, user1.address, underlyingToken.target, ethers.parseEther('100')),
      ).to.be.revert(ethers)
    })
  })

  describe('Exercise Call', function () {
    it('should allow exercising a call option (1/4)', async function () {
      const auctionInitialization: DataTypes.AuctionInitialization = {
        underlyingToken: String(underlyingToken.target),
        settlementToken: String(settlementToken.target),
        notional: ethers.parseEther('100'),
      ...
      await router.connect(user1).bidOnAuction(escrowAddress, user1.address, relBid, refSpot, data)
      ...
      await settlementToken.connect(user1).approve(router.target, ethers.parseEther('10'))
    ...
    it('should allow exercising a call option (2/4)', async function () {
      ...
      const notionalAmount = ethers.parseUnits('1050000', underlyingTokenDecimals)
      ...
      const rfqInitialization = await getRFQInitialization({
        ...
        underlyingTokenAddress: String(underlyingToken.target),
        settlementTokenAddress: String(settlementToken.target),
        premium: ethers.parseUnits('6674.757', settlementTokenDecimals),
        borrowCap: BASE,
        tenor: tenor,
      })
      ...
      await settlementToken.connect(user1).approve(router.target, ethers.MaxUint256)
      await underlyingToken.connect(owner).approve(router.target, ethers.MaxUint256)
      ...
      expect(Number(optionInfo.expiry) - Number(currentTime)).to.be.closeTo(tenor, 15) // Allowing a delta of 15 seconds

      // Approve settlement token for exercise
      await settlementToken.connect(user1).approve(router.target, ethers.MaxUint256)

      const expectedConversionAmount = (notionalAmount * strike) / 10n ** underlyingTokenDecimals

      // Check pre balances
      const preUndBalUser = await underlyingToken.balanceOf(user1.address)
      const preSettlementBalUser = await settlementToken.balanceOf(user1.address)
      const preUndBalOwner = await underlyingToken.balanceOf(owner.address)
      const preSettlementBalOwner = await settlementToken.balanceOf(owner.address)
      const preUndBalEscrow = await underlyingToken.balanceOf(escrow.target)
      const preSettlementBalEscrow = await settlementToken.balanceOf(escrow.target)
      ...
      const postSettlementBalUser = await settlementToken.balanceOf(user1.address)
      const postUndBalOwner = await underlyingToken.balanceOf(owner.address)
      const postSettlementBalOwner = await settlementToken.balanceOf(owner.address)
      const postUndBalEscrow = await underlyingToken.balanceOf(escrow.target)
      const postSettlementBalEscrow = await settlementToken.balanceOf(escrow.target)

      expect(postUndBalUser - preUndBalUser).to.be.equal(notionalAmount)
      expect(preSettlementBalUser - postSettlementBalUser).to.be.equal(expectedConversionAmount)
      expect(preUndBalOwner).to.be.equal(postUndBalOwner)
      expect(postSettlementBalOwner - preSettlementBalOwner).to.be.equal(expectedConversionAmount)
    ...
    it('should allow exercising a call option - large notional and small strike (3/4)', async function () {
      const underlyingTokenDecimals = await underlyingToken.decimals()
      const settlementTokenDecimals = await settlementToken.decimals()
      const BASE = ethers.parseEther('1')
      const tenor = 60 * 60 * 24 * 7
      const notionalAmount = ethers.parseUnits(String(100000000 / 0.000001), underlyingTokenDecimals) // 100m of pepe assuming pepe at $0.000001
      ...
      const rfqInitialization = await getRFQInitialization({
        ...
        underlyingTokenAddress: String(underlyingToken.target),
        settlementTokenAddress: String(settlementToken.target),
        premium: ethers.parseUnits('4000000', settlementTokenDecimals),
        borrowCap: BASE,
        tenor: tenor,
      })
      ...
      const expectedConversionAmount = (notionalAmount * strike) / 10n ** underlyingTokenDecimals
      await settlementToken.mint(user1.address, rfqInitialization.rfqQuote.premium + expectedConversionAmount)
    ...
    it('should allow exercising a put option (4/4)', async function () {
      const usdcDecimals = await settlementToken.decimals()
      const undDecimals = await underlyingToken.decimals()
      ...
      const rfqInitialization = await getRFQInitialization({
        notionalAmount: notionalAmount,
        strike: strike,
        underlyingTokenAddress: String(settlementToken.target),
        settlementTokenAddress: String(underlyingToken.target),
        premium: ethers.parseUnits('60674.757', usdcDecimals),
        borrowCap: BASE,
        tenor: tenor,
        premiumTokenIsUnderlying: true,
      })
      ...
      await settlementToken.connect(user1).approve(router.target, ethers.MaxUint256)
      await settlementToken.connect(owner).approve(router.target, ethers.MaxUint256)
      ...
      // Approve settlement token for exercise
      await underlyingToken.connect(user1).approve(router.target, ethers.MaxUint256)

      const expectedConversionAmount = (notionalAmount * strike) / 10n ** usdcDecimals

      // Check pre balances
      const preUndBalUser = await settlementToken.balanceOf(user1.address)
      const preSettlementBalUser = await underlyingToken.balanceOf(user1.address)
      const preUndBalOwner = await settlementToken.balanceOf(owner.address)
      const preSettlementBalOwner = await underlyingToken.balanceOf(owner.address)
      const preUndBalEscrow = await settlementToken.balanceOf(escrow.target)
      const preSettlementBalEscrow = await underlyingToken.balanceOf(escrow.target)
      ...
      const postSettlementBalUser = await underlyingToken.balanceOf(user1.address)
      const postUndBalOwner = await settlementToken.balanceOf(owner.address)
      const postSettlementBalOwner = await underlyingToken.balanceOf(owner.address)
      const postUndBalEscrow = await settlementToken.balanceOf(escrow.target)
      const postSettlementBalEscrow = await underlyingToken.balanceOf(escrow.target)
  ...
  describe('Borrow and Repay', function () {
    it('should allow borrowing and repaying (1/5)', async function () {
      const auctionInitialization: DataTypes.AuctionInitialization = {
        underlyingToken: String(underlyingToken.target),
        settlementToken: String(settlementToken.target),
        notional: ethers.parseEther('100'),
      ...
      const underlyingTokenDecimals = await underlyingToken.decimals()
      const expectedCollatAmount = (strike * underlyingBorrowAmount) / 10n ** underlyingTokenDecimals
      ...
      await expect(router.connect(user1).borrow(escrowAddress, user1.address, ethers.parseEther('10'))).to.emit(
        router,
        'Borrow',
      )
      ...
      expect(preSettlementBal - postSettlementBal).to.be.equal(expectedCollatAmount)

      // Check borrowed amount
      expect(await escrow.borrowedUnderlyingAmounts(user1.address)).to.equal(ethers.parseEther('10'))

      // Approve underlying token for repayment
      await underlyingToken.connect(user1).approve(router.target, ethers.parseEther('100'))

      // Repay borrowed amount
      await expect(router.connect(user1).repay(escrowAddress, user1.address, ethers.parseEther('10'))).to.emit(
        router,
        'Repay',
      )

      // Check borrowed amount after repayment
      expect(await escrow.borrowedUnderlyingAmounts(user1.address)).to.equal(ethers.parseEther('0'))
    })

    it('should allow borrowing and repaying (2/5)', async function () {
      const underlyingTokenDecimals = await underlyingToken.decimals()
      const settlementTokenDecimals = await settlementToken.decimals()
      const BASE = ethers.parseEther('1')
      const rfqInitialization = await getRFQInitialization({
      ...
      // Borrow underlying tokens
      await expect(router.connect(user1).borrow(escrowAddress, user1.address, underlyingBorrowAmount)).to.emit(
        router,
        'Borrow',
      )
      ...
      expect(postUndBal - preUndBal).to.be.equal(underlyingBorrowAmount)
      expect(preSettlementBal - postSettlementBal).to.be.equal(expectedCollatAmount)
      expect(postSettlementBalEscrow - preSettlementBalEscrow).to.be.equal(expectedCollatAmount)

      // Check escrow owner cannot withdraw collateral pre expiry
      await expect(escrow.handleWithdraw(owner.address, settlementToken.target, expectedCollatAmount)).to.be.revert(
        ethers,
      )

      // Check borrowed amount
      expect(await escrow.borrowedUnderlyingAmounts(user1.address)).to.equal(underlyingBorrowAmount)

      // Approve underlying token for repayment
      await underlyingToken.connect(user1).approve(router.target, underlyingBorrowAmount)
      ...
      // Repay borrowed amount
      await expect(router.connect(user1).repay(escrowAddress, user1.address, underlyingBorrowAmount)).to.emit(
        router,
        'Repay',
      )
      ...
      expect(postSettlementBal2 - preSettlementBal2).to.be.equal(expectedCollatAmount)

      // Check borrowed amount after repayment
      expect(await escrow.borrowedUnderlyingAmounts(user1.address)).to.equal(ethers.parseEther('0'))
    })

    it('should allow borrowing and repaying - partial borrow (3/5)', async function () {
      const underlyingTokenDecimals = await underlyingToken.decimals()
      const settlementTokenDecimals = await settlementToken.decimals()
      const BASE = ethers.parseEther('1')
      const notional = ethers.parseUnits('250000', underlyingTokenDecimals)
      const rfqInitialization = await getRFQInitialization({
      ...
      const optionInfo = await escrow.optionInfo()
      const strike = optionInfo.strike
      const underlyingBorrowAmount = (rfqInitialization.optionInfo.notional * 3n) / 10n // partially borrow 30%
      const expectedCollatAmount = (strike * underlyingBorrowAmount) / 10n ** underlyingTokenDecimals
      ...
      await expect(
        escrow.handleWithdraw(owner.address, settlementToken.target, expectedCollatAmount),
      ).to.be.revertedWithCustomError(escrow, 'InvalidWithdraw')
      await expect(
        router.withdraw(escrow.target, owner.address, settlementToken.target, expectedCollatAmount),
      ).to.be.revertedWithCustomError(escrow, 'InvalidWithdraw')

      // Check owner cannot withdraw remaining underlying amount pre expiry
      await expect(
        escrow.handleWithdraw(owner.address, underlyingToken.target, notional - underlyingBorrowAmount),
      ).to.be.revertedWithCustomError(escrow, 'InvalidWithdraw')
      await expect(
        router.withdraw(escrow.target, owner.address, underlyingToken.target, notional - underlyingBorrowAmount),
      ).to.be.revertedWithCustomError(escrow, 'InvalidWithdraw')

      // Fast forward time to after expiry
      const currentTime = (await provider.getBlock('latest')).timestamp
      await ethers.provider.send('evm_increaseTime', [rfqInitialization.optionInfo.expiry - currentTime + 1])
      await ethers.provider.send('evm_mine', [])
      ...
      await escrow.handleWithdraw(owner.address, settlementToken.target, expectedCollatAmount)

      // Check owner can withdraw remaining underlying amount post expiry
      await escrow.handleWithdraw(owner.address, underlyingToken.target, notional - underlyingBorrowAmount)
    ...
    it('should allow borrowing and repaying - multiple times before expiry (4/5)', async function () {
      const underlyingTokenDecimals = await underlyingToken.decimals()
      const settlementTokenDecimals = await settlementToken.decimals()
      const BASE = ethers.parseEther('1')
      ...
      const borrowAmounts = [
        (rfqInitialization.optionInfo.notional * 5n) / 100n,
        (rfqInitialization.optionInfo.notional * 10n) / 100n,
        (rfqInitialization.optionInfo.notional * 15n) / 100n,
        (rfqInitialization.optionInfo.notional * 20n) / 100n,
        (rfqInitialization.optionInfo.notional * 25n) / 100n,
        (rfqInitialization.optionInfo.notional * 30n) / 100n,
        (rfqInitialization.optionInfo.notional * 35n) / 100n,
        (rfqInitialization.optionInfo.notional * 40n) / 100n,
        (rfqInitialization.optionInfo.notional * 45n) / 100n,
        (rfqInitialization.optionInfo.notional * 50n) / 100n,
        (rfqInitialization.optionInfo.notional * 55n) / 100n,
        (rfqInitialization.optionInfo.notional * 60n) / 100n,
        (rfqInitialization.optionInfo.notional * 65n) / 100n,
        (rfqInitialization.optionInfo.notional * 70n) / 100n,
        (rfqInitialization.optionInfo.notional * 75n) / 100n,
        (rfqInitialization.optionInfo.notional * 80n) / 100n,
        (rfqInitialization.optionInfo.notional * 85n) / 100n,
        (rfqInitialization.optionInfo.notional * 90n) / 100n,
        (rfqInitialization.optionInfo.notional * 95n) / 100n,
        (rfqInitialization.optionInfo.notional * 100n) / 100n,
      ]

      for (const underlyingBorrowAmount of borrowAmounts) {
        const expectedCollatAmount = (strike * underlyingBorrowAmount) / 10n ** underlyingTokenDecimals
        ...
        expect(postUndBal - preUndBal).to.be.equal(underlyingBorrowAmount)
        expect(preSettlementBal - postSettlementBal).to.be.equal(expectedCollatAmount)
        expect(preUndBalEscrow - postUndBalEscrow).to.be.equal(underlyingBorrowAmount)
        expect(postSettlementBalEscrow - preSettlementBalEscrow).to.be.equal(expectedCollatAmount)

        // Check borrowed amount
        expect(await escrow.totalBorrowed()).to.be.equal(underlyingBorrowAmount)
        expect(await escrow.borrowedUnderlyingAmounts(user1.address)).to.be.equal(underlyingBorrowAmount)

        // Check balances
        const preRepayUndBal = await underlyingToken.balanceOf(user1.address)
        const preRepaySettlementBal = await settlementToken.balanceOf(user1.address)
        const preRepayUndBalEscrow = await underlyingToken.balanceOf(escrow.target)
        const preRepaySettlementBalEscrow = await settlementToken.balanceOf(escrow.target)
        ...
        const postRepaySettlementBal = await settlementToken.balanceOf(user1.address)
        const postRepayUndBalEscrow = await underlyingToken.balanceOf(escrow.target)
        const postRepaySettlementBalEscrow = await settlementToken.balanceOf(escrow.target)

        expect(preRepayUndBal - postRepayUndBal).to.be.equal(underlyingBorrowAmount)
        expect(postRepaySettlementBal - preRepaySettlementBal).to.be.equal(expectedCollatAmount)
        expect(postRepayUndBalEscrow - preRepayUndBalEscrow).to.be.equal(underlyingBorrowAmount)
        expect(preRepaySettlementBalEscrow - postRepaySettlementBalEscrow).to.be.equal(expectedCollatAmount)
      }
      ...
      // Fast forward time to after expiry
      await ethers.provider.send('evm_increaseTime', [timeToExpiry + 1])
      await ethers.provider.send('evm_mine', [])

      for (const underlyingBorrowAmount of borrowAmounts) {
        // Expect borrowing to revert
        await expect(
          router.connect(user1).borrow(escrowAddress, user1.address, underlyingBorrowAmount),
        ).to.be.revertedWithCustomError(escrow, 'InvalidBorrowTime')
      }
    })

    it('should allow borrowing and repaying - multiple times with large notional and small strike (5/5)', async function () {
      const underlyingTokenDecimals = await underlyingToken.decimals()
      const settlementTokenDecimals = await settlementToken.decimals()
      const BASE = ethers.parseEther('1')
      const tenor = 60 * 60 * 24 * 7
      ...
      const expectedMaxCollatAmount = (strike * notionalAmount) / 10n ** underlyingTokenDecimals

      await underlyingToken.mint(owner.address, notionalAmount)
      await settlementToken.mint(user1.address, expectedMaxCollatAmount + rfqInitialization.rfqQuote.premium)
    ...
    it('should revert if borrowing is not allowed', async function () {
      ...
      await expect(router.connect(user1).borrow(escrowAddress, user1.address, ethers.parseEther('10'))).to.be.revert(
        ethers,
      )
    })
    ...
    it('should allow withdrawing from expired auction and creating a new one (1/3)', async function () {
      ...
      await expect(
        router
          .connect(owner)
          .withdrawFromEscrowAndCreateAuction(user2.address, owner.address, auctionInitialization, ethers.ZeroAddress),
      ).to.be.revert(ethers)
      ...
      const postBalOldEscrow = await underlyingToken.balanceOf(oldEscrowAddress)
      ...
      await expect(router.connect(user1).bidOnAuction(newEscrowAddress, user1.address, relBid, refSpot, data)).to.emit(
        router,
        'BidOnAuction',
      )
    })
    ...
    it('should allow withdrawing from expired auction and creating a new one (2/3)', async function () {
      ...
      // Check balance changes
      expect(preBalUser - postBalUser).to.be.equal(newLargerNotional - oldNotional)
      expect(preBalOldEscrow).to.be.gt(0)
      expect(postBalOldEscrow).to.be.equal(0)
    ...
    it('should allow withdrawing from expired auction and creating a new one (3/3)', async function () {
      ...
      // Check balance changes
      expect(postBalUser - preBalUser).to.be.equal(oldNotional - newSmallerNotional)
      expect(preBalOldEscrow).to.be.gt(0)
      expect(postBalOldEscrow).to.be.equal(0)
  ...
  describe('Delegation', function () {
    it('should allow on-chain voting delegation', async function () {
      ...
      await expect(escrow.connect(user1).handleOnChainVoting(delegate)).to.be.revertedWithCustomError(
        escrow,
        'InvalidSender',
      )
    })
    ...
    it('should revert if delegation is not allowed', async function () {
      ...
      const delegate = user2.address
      await expect(escrow.connect(owner).handleOnChainVoting(delegate)).to.be.revertedWithCustomError(
        escrow,
  ...
  describe('Router handleOnChainVoting', function () {
    ...
    it('should allow the escrow owner to delegate on-chain voting via Router', async function () {
      ...
      await expect(router.connect(owner).handleOnChainVoting(escrow.target, delegate))
    ...
    it('should revert if non-owner calls router.handleOnChainVoting', async function () {
      ...
      await expect(router.connect(user1).handleOnChainVoting(escrow.target, delegate)).to.be.revertedWithCustomError(
        router,
        'InvalidSender',
      )
    })
    ...
    it('should revert if address is not a registered escrow', async function () {
      ...
      await expect(router.connect(owner).handleOnChainVoting(notAnEscrow, delegate)).to.be.revertedWithCustomError(
        router,
        'NotAnEscrow',
      )
    })
  ...
  describe('Set Fee Handler', function () {
    ...
    it('should revert if non-owner tries to set fee handler', async function () {
      ...
      await expect(router.connect(user1).setFeeHandler(newFeeHandler)).to.be.revertedWithCustomError(
        router,
    ...
    it('should revert if setting the same fee handler', async function () {
      ...
      await expect(router.connect(owner).setFeeHandler(currentFeeHandler)).to.be.revert(ethers)
    })
  })
  ...
  describe('Escrow initializeAuction', function () {
    it('should revert when re-initializing', async function () {
      ...
      await expect(
        escrow.initializeAuction(router.target, owner.address, 0, auctionInitialization, 1, ethers.ZeroAddress),
    ...
    it('should revert with InvalidTokenPair', async function () {
      ...
      await expect(
        router.connect(owner).createAuction(owner.address, auctionInitialization, ethers.ZeroAddress),
  ...
  describe('Escrow initializeRFQMatch', function () {
    it('should revert when re-initializing', async function () {
      ...
      await expect(
        escrow.initializeRFQMatch(router.target, owner.address, user1.address, 0, rfqInitialization, 1),
      ).to.be.revertedWithCustomError(escrowImpl, 'InvalidInitialization')
    })
  ...
  describe('Escrow handleAuctionBid and handleExercise', function () {
    ...
    describe('handleAuctionBid', function () {
      it('should revert with InvalidSender if not called by router', async function () {
        await expect(
          escrow
            .connect(user1)
            .handleAuctionBid(ethers.parseEther('0.1'), user1.address, ethers.parseUnits('1', 6), []),
      ...
      it('should revert on exercise without successful bid', async function () {
        await expect(
          router.connect(user1).exercise(escrow.target, user1.address, ethers.parseEther('1'), true, []),
    ...
    describe('handleExercise', function () {
      beforeEach(async function () {
        ...
        await router
          .connect(user1)
          .bidOnAuction(escrow.target, user1.address, ethers.parseEther('0.1'), ethers.parseUnits('1', 6), [])
      })

      it('should revert with InvalidSender if not called by router', async function () {
        await expect(
          escrow.connect(user1).handleExercise(user1.address, user1.address, ethers.parseEther('1'), true, []),
        ).to.be.revertedWithCustomError(escrow, 'InvalidSender')
      })
      ...
      it('should revert with InvalidExerciseAmount if amount is zero', async function () {
        ...
        await expect(
          router.connect(user1).exercise(escrow.target, user1.address, 0, true, []),
      ...
      it('should revert with InvalidExerciseAmount if amount exceeds notional', async function () {
        ...
        await expect(
          router.connect(user1).exercise(escrow.target, user1.address, optionInfo.notional + 1n, true, []),
        ).to.be.revertedWithCustomError(escrow, 'InvalidExerciseAmount')
      })
      ...
      it('should revert if exercising with underlying token but exercise cost is zero', async function () {
        ...
        await mockOracle.setPrice(settlementToken.target, underlyingToken.target, 1)

        await expect(
          router.connect(user1).exercise(escrow.target, user1.address, 1, false, []),
      ...
      it('should revert if exercising with underlying token but exercise cost is zero', async function () {
        ...
        const priceOfSettlementTokenInUnderlying =
          (ethers.parseUnits('1', underlyingTokenDecimals) * 10n ** settlementTokenDecimals) / optionInfo.strike
        ...
        await mockOracle.setPrice(settlementToken.target, underlyingToken.target, otmPrice)

        // Assume user wants to exercise on whole notional amount
        const exerciseCostInSettlementToken = (optionInfo.strike * optionInfo.notional) / 10n ** underlyingTokenDecimals
        const exerciseCostInUnderlyingToken =
          (exerciseCostInSettlementToken * otmPrice) / 10n ** settlementTokenDecimals
        ...
        await expect(
          router.connect(user1).exercise(escrow.target, user1.address, optionInfo.notional, false, []),
        ).to.be.revertedWithCustomError(escrow, 'InvalidExercise')
      })
      ...
      it('should successfully handle a valid exercise', async function () {
        ...
        await expect(
          router.connect(user1).exercise(escrow.target, user1.address, optionInfo.notional, true, []),
        ).to.emit(router, 'Exercise')
      })
    })
  })
  ...
  describe('Escrow handleBorrow', function () {
    ...
    it('should revert with InvalidSender if not called by router', async function () {
      await expect(
        escrow.connect(user1).handleBorrow(user1.address, user1.address, ethers.parseEther('1')),
      ).to.be.revertedWithCustomError(escrow, 'InvalidSender')
    })
    ...
    it('should revert with NoOptionMinted if option is not minted', async function () {
      ...
      const newEscrow = await createAuction(auctionInitialization, router, owner)

      await expect(
        router.connect(user1).borrow(newEscrow.target, user1.address, ethers.parseEther('1')),
    ...
    it('should revert with InvalidBorrowTime if borrowed too early', async function () {
      await expect(
        router.connect(user1).borrow(escrow.target, user1.address, ethers.parseEther('1')),
      ).to.be.revertedWithCustomError(escrow, 'InvalidBorrowTime')
    })
    ...
    it('should revert with InvalidBorrowAmount if amount is zero', async function () {
      ...
      await expect(router.connect(user1).borrow(escrow.target, user1.address, 0)).to.be.revertedWithCustomError(
        escrow,
        'InvalidBorrowAmount',
      )
    })
    ...
    it('should revert with InvalidBorrowAmount if amount exceeds borrow cap', async function () {
      ...
      const borrowCapExceeded =
        (optionInfo.notional * BigInt(auctionInitialization.advancedSettings.borrowCap)) / ethers.parseEther('1') + 1n

      await expect(
        router.connect(user1).borrow(escrow.target, user1.address, borrowCapExceeded),
    ...
    it('should successfully handle a valid borrow', async function () {
      ...
      const validBorrowAmount =
        (optionInfo.notional * BigInt(auctionInitialization.advancedSettings.borrowCap)) / ethers.parseEther('1')

      await settlementToken.mint(user1.address, ethers.parseEther('1000'))
      await settlementToken.connect(user1).approve(router.target, ethers.parseEther('1000'))

      await expect(router.connect(user1).borrow(escrow.target, user1.address, validBorrowAmount)).to.emit(
        router,
        'Borrow',
      )

      const borrowedAmount = await escrow.borrowedUnderlyingAmounts(user1.address)
  ...
  describe('Escrow handleRepay', function () {
    ...
    beforeEach(async function () {
      ...
      await router.connect(user1).borrow(escrow.target, user1.address, validBorrowAmount)
    })

    it('should revert with InvalidSender if not called by router', async function () {
      await expect(
        escrow.connect(user1).handleRepay(user1.address, user1.address, ethers.parseEther('1')),
      ).to.be.revertedWithCustomError(escrow, 'InvalidSender')
    })
    ...
    it('should revert with NoOptionMinted if option is not minted', async function () {
      ...
      await expect(
        router.connect(user1).repay(newEscrow.target, user1.address, ethers.parseEther('1')),
      ).to.be.revertedWithCustomError(newEscrow, 'NoOptionMinted')
    })
    ...
    it('should revert with InvalidRepayTime if repay called after expiry', async function () {
      ...
      await expect(
        router.connect(user1).repay(escrow.target, user1.address, ethers.parseEther('1')),
    ...
    it('should revert with InvalidRepayAmount if amount is zero', async function () {
      await expect(router.connect(user1).repay(escrow.target, user1.address, 0)).to.be.revertedWithCustomError(
        escrow,
        'InvalidRepayAmount',
      )
    })
    ...
    it('should revert with InvalidRepayAmount if amount exceeds borrowed amount', async function () {
      ...
      await expect(
        router.connect(user1).repay(escrow.target, user1.address, excessiveRepayAmount),
    ...
    it('should successfully handle a valid repay', async function () {
      ...
      await expect(router.connect(user1).repay(escrow.target, user1.address, repayAmount)).to.emit(router, 'Repay')

      const remainingBorrowedAmount = await escrow.borrowedUnderlyingAmounts(user1.address)
  ...
  describe('Escrow transfer ownership', function () {
    ...
    it('should allow the owner to transfer ownership to a new address', async function () {
      ...
      await expect(
        router.connect(owner).emitTransferOwnershipEvent(owner.address, newOwner.address),
    ...
    it('should revert if a non-owner tries to transfer ownership', async function () {
      ...
      await expect(escrow.connect(user1).transferOwnership(newOwner)).to.be.revertedWithCustomError(
        escrow,
        'InvalidSender',
      )
    })
    ...
    it('should revert if the new owner is the same as the current owner', async function () {
      ...
      await expect(escrow.connect(owner).transferOwnership(currentOwner)).to.be.revertedWithCustomError(
        escrow,
        'OwnerAlreadySet',
      )
    })
  ...
  describe('Escrow handleWithdraw', function () {
    ...
    it('should allow the owner to withdraw after option expiry', async function () {
      ...
      const userBalPreWithdrawal = await underlyingToken.balanceOf(user1.address)
      await expect(escrow.connect(owner).handleWithdraw(user1.address, underlyingToken.target, withdrawAmount))
        .to.emit(escrow, 'Withdraw')
        .withArgs(owner.address, user1.address, underlyingToken.target, withdrawAmount)
      const userBalPostWithdrawal = await underlyingToken.balanceOf(user1.address)

      // Check the balances to confirm the transfer
      expect(userBalPostWithdrawal - userBalPreWithdrawal).to.equal(withdrawAmount)
    })
    ...
    it('should revert if a non-owner or non-router tries to withdraw', async function () {
      ...
      const withdrawAmount = optionInfo.notional
      await expect(
        escrow.connect(user1).handleWithdraw(user1.address, underlyingToken.target, withdrawAmount),
      ).to.be.revertedWithCustomError(escrow, 'InvalidSender')
    })
    ...
    it('should revert if withdrawal is attempted before expiry when option is minted', async function () {
      ...
      await expect(
        escrow.connect(owner).handleWithdraw(owner.address, underlyingToken.target, withdrawAmount),
      ).to.be.revertedWithCustomError(escrow, 'InvalidWithdraw')
    })
  })
  ...
  describe('Escrow handleOffChainVoting', function () {
    ...
    beforeEach(async function () {
      ...
      const DelegateRegistry = await ethers.getContractFactory('MockDelegateRegistry')
    ...
    it('should allow the owner to delegate off-chain voting', async function () {
      ...
      await expect(escrow.connect(owner).handleOffChainVoting(spaceId, delegate))
      ...
      expect(await delegateRegistry.delegation(escrow.target, spaceId)).to.equal(delegate)
    })
    ...
    it('should revert if a non-owner tries to delegate', async function () {
      ...
      await expect(escrow.connect(user1).handleOffChainVoting(spaceId, delegate)).to.be.revertedWithCustomError(
        escrow,
        'InvalidSender',
      )
    })
    ...
    it('should revert if no allowed delegate registry is set', async function () {
      ...
      await expect(escrow.connect(owner).handleOffChainVoting(spaceId, delegate)).to.be.revertedWithCustomError(
        escrow,
  ...
  describe('Router handleOffChainVoting', function () {
    ...
    it('should allow the escrow owner to delegate off-chain voting via Router', async function () {
      ...
      await expect(router.connect(owner).handleOffChainVoting(escrow.target, spaceId, delegate))
    ...
    it('should revert if non-owner calls router.handleOffChainVoting', async function () {
      ...
      await expect(
        router.connect(user1).handleOffChainVoting(escrow.target, spaceId, delegate),
    ...
    it('should revert if address is not a registered escrow', async function () {
      ...
      await expect(
        router.connect(owner).handleOffChainVoting(notAnEscrow, spaceId, delegate),
      ).to.be.revertedWithCustomError(router, 'NotAnEscrow')
    })
  })
  ...
  describe('Edge Cases and Reverts', function () {
    ...
    it('should revert when non-escrow address tries to interact', async function () {
      ...
      await expect(
        router.connect(user1).withdraw(user2.address, user1.address, underlyingToken.target, ethers.parseEther('10')),
      ).to.be.revertedWithCustomError(router, 'NotAnEscrow')
    })
    ...
    it('should revert when bidding on non-existent escrow', async function () {
      ...
      await expect(
        router
          .connect(user1)
          .bidOnAuction(nonExistentEscrow, user1.address, ethers.parseEther('0.1'), ethers.parseUnits('1', 6), []),
      ).to.be.revert(ethers)
    })
    ...
    it('should revert when exercising on non-existent escrow', async function () {
      ...
      await expect(
        router.connect(user1).exercise(nonExistentEscrow, user1.address, ethers.parseEther('10'), false, []),
      ).to.be.revert(ethers)
    })
    ...
    it('should revert when borrowing from non-existent escrow', async function () {
      ...
      await expect(
        router.connect(user1).borrow(nonExistentEscrow, user1.address, ethers.parseEther('10')),
      ).to.be.revert(ethers)
    })
    ...
    it('should revert when repaying to non-existent escrow', async function () {
      ...
      await expect(router.connect(user1).repay(nonExistentEscrow, user1.address, ethers.parseEther('10'))).to.be.revert(
        ethers,
      )
    })
    ...
    it('should revert with InvalidGetEscrowsQuery for invalid queries', async function () {
      ...
      await expect(router.getEscrows(0, 0)).to.be.revertedWithCustomError(router, 'InvalidGetEscrowsQuery')

      // Case 2: from + numElements > length
      await expect(router.getEscrows(2, 3)).to.be.revertedWithCustomError(router, 'InvalidGetEscrowsQuery')
